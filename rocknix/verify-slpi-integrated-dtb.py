#!/usr/bin/env python3
"""Verify the integrated RP5 DT is Phase 6D plus SLPI and its handshake."""
...
def main() -> int:
    if len(sys.argv) != 3:
        raise SystemExit("usage: verify-slpi-integrated-dtb.py PHASE6D_DTB INTEGRATED_DTB")
    ...
    if symbol(baseline, "slpi") != slpi_path:
        raise SystemExit("SLPI symbol changed between baseline and candidate")
    metadata = {"phandle", "linux,phandle"}
    allowed_change = (slpi_path, "status")
    baseline_existing = {
        key: value for key, value in baseline.items()
        if key[0] != "/__symbols__" and key[1] not in metadata and key != allowed_change
    }
    candidate_existing = {
        key: value for key, value in candidate.items()
        if key[0] != "/__symbols__" and key[0] not in new_nodes
        and key[1] not in metadata and key != allowed_change
    }
    ...
    if baseline.get((slpi_path, "status")) != one_string("disabled"):
        failures.append("baseline-slpi/status")
    if candidate.get((slpi_path, "status")) != one_string("okay"):
        failures.append("candidate-slpi/status")
    ...
    if failures:
        raise SystemExit("Integrated DT verification failed: " + ", ".join(failures))

    print("Verified exact Phase 6D DT plus enabled SLPI and three sleep-state nodes")
