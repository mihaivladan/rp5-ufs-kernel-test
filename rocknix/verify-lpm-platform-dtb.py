#!/usr/bin/env python3
"""Verify Phase 6K is byte-semantic Phase 6J plus one LPM suspend node."""
...
def main() -> int:
    ...
    required = {
        (node, "compatible"): one_string("qcom,consoleos-lpm-suspend"),
    ...
    if actual_node_properties != required:
        raise SystemExit(
            "LPM node mismatch: "
            f"expected={sorted(required)}, actual={sorted(actual_node_properties)}"
        )

    print("Verified exact Phase 6J DT plus one platform-suspend node")
    print("Verified exact composite PSCI state 0x4100c244 and no other semantic delta")
    return 0
