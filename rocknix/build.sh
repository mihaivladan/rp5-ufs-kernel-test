...
if [[ "${profile}" == minimal-sleep ]]; then
    ...
    printf '%s\n' \
        'RP5 stock-kernel re-enable matrix passed: proven baseline plus eleven exact subsystem candidates and eleven cumulative integration candidates, including Phase 6A DisplayPort-only, Phase 6B ADSP/LPASS and Phase 6C LPASS-only discriminators plus the Phase 6 audio candidate with active-only UART6 plus UART16; CPU ICC removal fixed.' \
        > BUILD-SUCCESS.txt
    cd "${source_dir}"
fi
