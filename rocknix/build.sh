...
if [[ "${profile}" == minimal-sleep ]]; then
    ...
    printf '%s\n' \
        'RP5 stock-kernel re-enable matrix passed: proven baseline plus eleven exact subsystem candidates and seven cumulative integration candidates, including the Phase 5 active-only-gamepad-plus-UFS-wireless-USB-Type-C candidate; CPU ICC removal fixed.' \
        > BUILD-SUCCESS.txt
    cd "${source_dir}"
fi
