...
if sys.argv[2] == 'minimal-sleep':
    ...
    absent = sorted(required - actual)
    if absent:
        raise SystemExit('PCIe DRV handoff requirements missing: ' + ', '.join(absent))
    pas = Path('drivers/remoteproc/qcom_q6v5_pas.c').read_text()
    start = pas.index('static const struct qcom_pas_data sm8250_adsp_resource = {')
    end = pas.index('\n};', start)
    block = pas[start:end]
...
if [[ "${profile}" == minimal-sleep ]]; then
    ...
    printf '%s\n' \
        ...
        > BUILD-SUCCESS.txt
    cd "${source_dir}"
elif [[ "${profile}" == pcie-drv-handoff ]]; then
    ...
    cp "arch/arm64/boot/dts/qcom/${dtb_name}.dtb" "${out}/"
    cd "${out}"
    sha256sum "${dtb_name}.dtb" > PCIE-DRV-DTB-SHA256SUMS
    printf '%s\n' \
...
release=$(make "${make_args[@]}" -s kernelrelease)
test "${release}" = "${expected_release}"
if [[ "${profile}" == diagnostic || "${profile}" == gpu-rpmh-fix || "${profile}" == sleepstate-handshake || "${profile}" == adsp-no-auto-ab || "${profile}" == pcie-drv-handoff || "${profile}" == slpi-integrated || "${profile}" == lpm-platform ]]; then
...
if [[ "${profile}" == gpu-rpmh-fix || "${profile}" == sleepstate-handshake || "${profile}" == adsp-no-auto-ab || "${profile}" == pcie-drv-handoff || "${profile}" == slpi-integrated || "${profile}" == lpm-platform ]]; then
...
if [[ "${profile}" == adsp-no-auto-ab || "${profile}" == pcie-drv-handoff || "${profile}" == slpi-integrated || "${profile}" == lpm-platform ]]; then
    cp drivers/remoteproc/qcom_q6v5_pas.c "${out}/"
fi
if [[ "${profile}" == lpm-platform ]]; then
    cp drivers/soc/qcom/qcom_lpm_platform_suspend.c "${out}/"
fi
if [[ "${profile}" == pcie-drv-handoff ]]; then
