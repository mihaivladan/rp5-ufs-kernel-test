...
def main() -> int:
    ...
    if manifest.get("schema") != 2 or len(manifest.get("candidates", [])) != 23:
    ...
    phase6d = candidate_properties["display-gpu-gamepad-active-only-ufs-wireless-usb-typec-dp-adsp"]
    phase6d_delta = {
        key for key in phase6a.keys() | phase6d.keys()
        if phase6a.get(key) != phase6d.get(key)
    }
    expected_phase6d_delta = {(resolve("adsp", baseline), "status")}
    if phase6d_delta != expected_phase6d_delta:
        raise SystemExit(
            f"Phase 6D is not an exact ADSP-status delta from Phase 6A: "
            f"actual={sorted(phase6d_delta)} expected={sorted(expected_phase6d_delta)}"
        )
    phase6b_from_6d_delta = {
        key for key in phase6d.keys() | phase6b.keys()
        if phase6d.get(key) != phase6b.get(key)
    }
    expected_phase6b_from_6d_delta = {(resolve("lpass_tlmm", baseline), "status")}
    if phase6b_from_6d_delta != expected_phase6b_from_6d_delta:
        raise SystemExit(
            f"Phase 6B is not an exact LPASS-status delta from Phase 6D: "
            f"actual={sorted(phase6b_from_6d_delta)} expected={sorted(expected_phase6b_from_6d_delta)}"
        )
    ...
    print("Verified Phase 6D relative delta: exactly ADSP disabled-to-okay; Phase 6B then adds exactly LPASS pinctrl")
    ...
    print("Verified 11 independent subsystem candidates plus 12 cumulative integration candidates")
    return 0
